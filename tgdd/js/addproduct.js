class Product {
    constructor(name, auchor, price, NXB, NPH, category, size, page, weight, img, intro) {
        this.name = name;
        this.auchor = auchor;
        this.price = price;
        this.NXB = NXB;
...
$(document).ready(() => {
    $("#formAddProduct button").on("click", () => {
        ...
        arrOB2.forEach((element) => {
            element.item == ""
                ? ($(`#formAddProduct p:eq(${element.index})`).html("Please enter this field").css({
                      color: "red",
                      //   "margin-bottom": "6px",
                  }),
        ...
        $("#formAddProduct input:eq(2)").val().match(",") || $("#formAddProduct input:eq(2)").val().match(".")
            ? ($("#formAddProduct input:eq(2)").val(
        ...
        !$("#formAddProduct input:eq(2)").val().match(",")
            ? $("#formAddProduct input:eq(2)").val(
                  parseInt($("#formAddProduct input:eq(2)").val()).toLocaleString().replaceAll(",", "."),
              )
            : "";
        ...
        document.querySelector("textarea").onchange = () => {
            $("#formAddProduct textarea").val() != "" ? $("#formAddProduct p:eq(10)").html("") : "";
        };
